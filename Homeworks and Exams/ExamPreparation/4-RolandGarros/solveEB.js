...
function solve(input) {
    ...
    input = input.map(function(inputLine) {
        return inputLine.replace(/\s{2,}/g, ' ');
    });

    input.forEach(function(inputLine) {
        var inputTokens = inputLine.split(':');
        var playerData          = inputTokens[0].split('vs.'),
            firstPlayer         = playerData[0].trim(),
            secondPlayer        = playerData[1].trim(),
            firstPlayerGames    = 0,
            firstPlayerSets     = 0,
        ...
        var resultData = inputTokens[1].trim()
                                       .split(' ')
                                       .forEach(function(setResult) {
                                           var gameInfo = setResult.split('-')
                                                                   .map(function(str) {
                                                                       return parseInt(str);
                                                                   });
        ...
        updatePlayer(firstPlayer, firstPlayerMatches, secondPlayerMatches, firstPlayerSets, secondPlayerSets, firstPlayerGames, secondPlayerGames);
        updatePlayer(secondPlayer, secondPlayerMatches, firstPlayerMatches, secondPlayerSets, firstPlayerSets, secondPlayerGames, firstPlayerGames);
