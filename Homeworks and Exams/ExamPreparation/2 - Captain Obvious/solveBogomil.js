"use strict";
function solve(arr) {
    // Bogomil polzva i niakakakav framework v koito imashe Array.prototype.occures(az ia napisah), no spored men ne e e dobar variant, tai kato
    // ste varti cikli za vsiako izrecehenie tolkova pati, kolkoto sa dumite v repeating words- vij komentara otdulou
    ...
    // counting repeating words in first input
    var words = {};
    var firstSequence = arr[0];
    var matches = firstSequence.toLowerCase().match(/[a-zA-Z]+/g);
    var repeatingWords=[];
    matches.forEach(function(word) {
        if (matches.occurs(word) >= 3){
            repeatingWords[word]=undefined;
        }
    });
    repeatingWords=Object.keys(repeatingWords);
    if (repeatingWords.length === 0) {
        console.log('No words');
        return;
    }

    // checking for words in the second input
    var regex = /[\w].+?[?!.]/g;
    var match;
